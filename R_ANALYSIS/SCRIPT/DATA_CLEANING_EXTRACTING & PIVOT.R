...
library(readr)
#n READ THE TXT FILE
DATA <- read_delim("E:/SAMPLE_DATA/LOAN_CLIENT_BASE_DATA.txt",sep="|",header=TRUE)
...
DATA$ACCT_CLOS_DTE[DATA$ACCT_CLOS_DTE=="NULL"] <- "NA"

### EXTRACT THE CLOSED ACCOUNT DETAILS
CLOSED <- na_omit(DATA, c("ARH$ACCT_CLOS_DTE"))
...
active=sqldf('SELECT * FROM DATA WHERE LGR_BAL >"0" AND DISB_AMT >"0"')
