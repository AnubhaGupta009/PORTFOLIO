...
DESCRIBE dataset;
SELECT * FROM dataset;
SELECT patient_id,gender,icu_type
FROM dataset 
WHERE gender = 'F' AND icu_type = 'CTICU';
------ patient_survival_data (dataset)
UPDATE dataset SET ethnicity = CASE 
    WHEN ethnicity = '' THEN 'Mixed'
    ELSE ethnicity
    END;

-- How many total deaths occured in the hospital and what was the percentage of the mortality rate of each gender?
SELECT gender,COUNT(CASE WHEN hospital_death = 1 THEN 1 END) AS total_hospital_deaths, 
ROUND(COUNT(CASE WHEN hospital_death = 1 THEN 1 END)*100/COUNT(*),2) AS mortality_rate
FROM dataset GROUP BY gender;
...
-- Comparing the average and max ages of patients who died and the patients who survived
...
-- Comparing the count of patients that died and survived by each age 
SELECT age,
	COUNT(CASE WHEN hospital_death = '1' THEN 1 END) as died_patients_count,
	COUNT(CASE WHEN hospital_death = '0' THEN 1 END) as survived_patients_count,
    ROUND(COUNT(CASE WHEN hospital_death = '1' THEN 1 END) *100/
    COUNT(*),2) AS died_percentage
FROM dataset
GROUP BY age
ORDER BY age ASC;
...
-- count of patients above 65 who died vs count of patients between 50-65 who died
...
-- Which admit source have greatest death % ?
SELECT DISTINCT icu_admit_source,
	COUNT(CASE WHEN hospital_death = '1' THEN 1 END) as died_patient_count,
	COUNT(CASE WHEN hospital_death = '0' THEN 1 END) as survived_patient_count,
	ROUND(COUNT(CASE WHEN hospital_death = '1' THEN 1 END) *100/
    COUNT(*),2) AS died_percentage
FROM dataset
GROUP BY icu_admit_source;

-- Average age of people in each ICU admit source and died count
SELECT DISTINCT icu_admit_source,
	COUNT(hospital_death) as died_count,
	ROUND(AVG(age),2) as avg_age
FROM dataset
WHERE hospital_death = '1'
GROUP BY icu_admit_source;

-- gender wise Average weight, bmi, and max heartrate of people who died
SELECT Gender,ROUND(AVG(weight),2) as avg_weight,
...
-- What was the gender wise percentage of patients with each comorbidity among those who died? 
SELECT gender,
...
-- What were the top 10 ICUs (id) with the highest hospital death probability? 
SELECT icu_id, 
...
-- What was the average days of stay at each ICU for patients who survived and those who didn't? 
SELECT
    icu_type,
    ROUND(AVG(CASE WHEN hospital_death = 1 THEN pre_icu_los_days END), 2) AS avg_icu_days_stay_death,
    ROUND(AVG(CASE WHEN hospital_death = 0 THEN pre_icu_los_days END), 2) AS avg_icu_days_stay_survived
FROM dataset
GROUP BY icu_type
ORDER BY icu_type;
...
SELECT ethnicity,
    ROUND(COUNT(CASE WHEN hospital_death = 1 THEN 1 END) * 100 / (SELECT COUNT(*) FROM dataset), 2) AS death_percentage
FROM dataset
GROUP BY ethnicity;
...
-- Hospital top 5 death probabilities where the ICU type is 'SICU' and BMI is above 30
SELECT
    patient_id,
    apache_4a_hospital_death_prob as hospital_death_prob
FROM dataset
